...
describe("useUILayoutStore", () => {
  ...
  describe("sessions sidebar", () => {
    it("toggleSessionsSidebar toggles the collapsed state", () => {
      expect(useUILayoutStore.getState().sessionsSidebarCollapsed).toBe(false);

      act(() => {
        useUILayoutStore.getState().toggleSessionsSidebar();
      });
      expect(useUILayoutStore.getState().sessionsSidebarCollapsed).toBe(true);
  ...
  describe("chat sidebar", () => {
    it("toggleChatSidebar toggles the collapsed state", () => {
      expect(useUILayoutStore.getState().chatSidebarCollapsed).toBe(false);
  ...
  describe("mobile sheets", () => {
    it("setMobileSessionsSheetOpen updates state", () => {
      ...
      expect(useUILayoutStore.getState().mobileSessionsSheetOpen).toBe(true);

      act(() => {
        useUILayoutStore.getState().setMobileSessionsSheetOpen(false);
      });
      expect(useUILayoutStore.getState().mobileSessionsSheetOpen).toBe(false);
    });
  ...
  describe("collapsible sections", () => {
    it("setSectionCollapsed sets a section to collapsed", () => {
      act(() => {
        useUILayoutStore.getState().setSectionCollapsed("test-section", true);
      });
    ...
    it("toggleSection toggles section collapsed state", () => {
      act(() => {
        useUILayoutStore.getState().setSectionCollapsed("toggle-test", false);
      });
      expect(useUILayoutStore.getState().collapsedSections["toggle-test"]).toBe(false);

      act(() => {
        useUILayoutStore.getState().toggleSection("toggle-test");
      });
      expect(useUILayoutStore.getState().collapsedSections["toggle-test"]).toBe(true);
    ...
    it("isSectionCollapsed returns correct state for known sections", () => {
      act(() => {
        useUILayoutStore.getState().setSectionCollapsed("known-section", true);
      });

      expect(useUILayoutStore.getState().isSectionCollapsed("known-section")).toBe(true);
    });

    it("isSectionCollapsed uses defaultOpen for unknown sections", () => {
      // Default open = true means collapsed = false
      expect(useUILayoutStore.getState().isSectionCollapsed("unknown-section", true)).toBe(false);

      // Default open = false means collapsed = true
      expect(useUILayoutStore.getState().isSectionCollapsed("another-unknown", false)).toBe(true);
    });
  });
});
