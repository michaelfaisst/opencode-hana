...
describe("useNotificationStore", () => {
    ...
    describe("initial state", () => {
        it("has notifications enabled by default", () => {
            expect(useNotificationStore.getState().notificationsEnabled).toBe(
                true
            );
        });
        ...
        it("has browser notifications enabled by default", () => {
            expect(
                useNotificationStore.getState().browserNotificationsEnabled
            ).toBe(true);
        });
        ...
        it("has notification-1 as default sound", () => {
            expect(useNotificationStore.getState().selectedSound).toBe(
                "notification-1"
            );
        });
    });
    ...
    describe("setSelectedSound", () => {
        it("updates selectedSound state", () => {
            act(() => {
                useNotificationStore
                    .getState()
                    .setSelectedSound("notification-3");
            });
            expect(useNotificationStore.getState().selectedSound).toBe(
                "notification-3"
            );
    ...
    describe("setBrowserPermission", () => {
        it("updates browserPermission state", () => {
            ...
            expect(useNotificationStore.getState().browserPermission).toBe(
                "granted"
            );
