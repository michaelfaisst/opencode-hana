...
export function McpServersDialog({
    ...
    const toggleServer = useCallback(
        (server: McpServer) => {
            ...
            if (connectMutation.isPending || disconnectMutation.isPending)
                return;
        ...
        (e: React.KeyboardEvent) => {
            ...
            switch (e.key) {
                ...
                case "ArrowUp":
                    e.preventDefault();
                    setSelectedIndex(
                        (prev) => (prev - 1 + servers.length) % servers.length
                    );
                    break;
                case " ":
                    e.preventDefault();
...
const McpServerItem = forwardRef<HTMLDivElement, McpServerItemProps>(
    function McpServerItem(
        ...
        const { icon, label, className, isConnected } = getStatusInfo(
            server.status
        );
