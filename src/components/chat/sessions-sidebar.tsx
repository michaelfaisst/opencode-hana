...
export function SessionsSidebar({ forceExpanded, onSessionSelect }: SessionsSidebarProps) {
  ...
  const filteredSessions = useMemo(() => {
    ...
    return sortedSessions.filter((session) => {
      ...
      const projectName = session.directory ? getProjectName(session.directory).toLowerCase() : "";
  ...
  const sessionGroups = useMemo(() => {
    ...
    return Array.from(groups.values()).sort((a, b) => b.mostRecentUpdate - a.mostRecentUpdate);
            ...
            {isLoading ? (
              ...
              sessionGroups.map((group) => (
                <SessionGroupComponent
                  key={group.directory}
                  group={group}
                  isCollapsed={collapsedGroups.has(group.directory)}
